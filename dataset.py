...
class Dataset:
    ...
    def _load_dataset(self, shuffle=True):
        ...
        def _load_imgs(path, fn, regx='.*.tif', printable=False, **kwargs):
            ...
            ##changing dtype from float32 to lower-precision to save ram (ts, 5/6/21)
            #bfloat16=tf.bfloat16.as_numpy_dtype
            for img_file in img_list:
                img = fn(img_file, path, **kwargs)
                #if (img.dtype != bfloat16):
                img = img.astype(np.uint8, casting='unsafe') #bfloat16
                print('\r %s -- %s' % (img_file, str(img.shape)),end='\r')
                imgs.append(img)
            print()
