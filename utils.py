...
def normalize_percentile(im, low=0.1, high=99.9,clip=False,is_random=False):
    if is_random:
...
def _write3d(x, path, bitdepth=16, norm_max=True):
    ...
    if bitdepth == 32:
        ...
        if bitdepth == 8:
            scale_x = x * 255
            if len(np.where(scale_x>255)[0])!=0:
                scale_x=_min_max(x)*255
            scale_x = scale_x.astype(np.uint8)
        else:
            scale_x = x * 65535
            if len(np.where(scale_x>65535)[0])!=0:
                scale_x=_min_max(x)*65535
            scale_x = scale_x.astype(np.uint16)
    imageio.volwrite(path, scale_x[..., 0])
