...
def infer(epoch, batch_size=1, use_cpu=False, save_pb=False):
    ...
    with tf.device(device_str): 
        if model_type is 'function':
            net = UNet_B(t_image, n_slices, [height * n_num, width * n_num], is_train=True, reuse=tf.AUTO_REUSE, name='unet')
        else:
            net = UNet_A(t_image, n_slices, [height * n_num, width * n_num], is_train=True, reuse=tf.AUTO_REUSE, name='unet')
        if model_type is 'function':
            net = UNet_B(t_image, n_slices, [height * n_num, width * n_num], is_train=True, reuse=tf.AUTO_REUSE, name='unet')
        elif model_type is 'structure':
            net = UNet_A(t_image, n_slices, [height * n_num, width * n_num], is_train=True, reuse=tf.AUTO_REUSE, name='unet')
        elif 'beta' in model_type :
            net = UNet_C(t_image, n_slices=n_slices, output_size=[height * n_num, width * n_num], is_train=True, reuse=tf.AUTO_REUSE,name='unet',normalize_mode=normalize_mode)
